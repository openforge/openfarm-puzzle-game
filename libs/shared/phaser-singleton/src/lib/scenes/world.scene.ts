...
import { GameInstanceService, VibrationService } from '@openforge/shared/data-access-model';
import { AchievementsPlugin } from '@openforge/shared-phaser-singleton';
...
export class WorldScene extends Phaser.Scene {
    ...
    async preload(): Promise<void> {
        try {
            ...
            this.load.atlas('animals', 'assets/puzzle-game/animals.png', 'assets/puzzle-game/animals_atlas.json');
            this.load.svg('back-fence', 'assets/puzzle-game/back_fence.svg', { scale: 1 });
            this.load.svg('feed', 'assets/puzzle-game/feed.svg', { scale: 1 });
            this.load.svg('feedbox', 'assets/puzzle-game/feedbox.svg', { scale: 1 });
            this.load.svg('front-fence', 'assets/puzzle-game/front_fence.svg', { scale: 1 });
            this.load.svg('refresh-sign', 'assets/puzzle-game/refresh_sign.svg', { scale: 1 });
            this.load.svg('title', 'assets/puzzle-game/title_level_score.svg', { scale: 1 });
            this.load.image('match-particle', 'assets/puzzle-game/white_particle.png');
        } catch (e) {
    ...
    create() {
        ...
        const restartSign = this.add
            .image(this.game.scale.gameSize.width - 5, this.game.scale.gameSize.height, 'refresh-sign')
            .setOrigin(1)
        ...
        const powerUpEmitter$ = this.gameInstanceService.powerUpEmitter$ as Observable<void>;
    ...
    private getPowerups() {
        ...
        this.bombs = [];
        const bombs = this.gameInstanceService.bombPowerUps;
        for (let i = 0; i < bombs; i++) {
            const bomb = this.add
                .image((i * this.tileWidth * 1.5) / 2 + (this.tileWidth * 1.5) / 3, this.game.scale.height - 5 - (this.feedbox.height * this.feedbox.scale) / 2, 'feed')
    ...
    private tileDown(tile: Phaser.GameObjects.Sprite) {
        const vibrationService = (this.gameInstanceService as any).vibrationService as VibrationService;
        if (Capacitor.getPlatform() !== 'web') {
            void vibrationService.giveHapticFeedback();
        }
        if (this.canMove) {
            ...
            for (j = 0; j < tempArr.length; j++) {
                if (j < tempArr.length - 2) {
                    if (grid[i][j] && grid[i][j + 1] && grid[i][j + 2]) {
                        if (grid[i][j].tileType === grid[i][j + 1].tileType && grid[i][j + 1].tileType === grid[i][j + 2].tileType) {
                            if (groups.length > 0) {
            ...
            for (i = 0; i < tempArr.length; i++) {
                if (i < tempArr.length - 2) {
                    if (grid[i][j] && grid[i + 1][j] && grid[i + 2][j]) {
                        if (grid[i][j].tileType === grid[i + 1][j].tileType && grid[i + 1][j].tileType === grid[i + 2][j].tileType) {
                            if (groups.length > 0) {
                                if (groups.indexOf(grid[i][j]) === -1) {
                                    matches.push(groups);
                                    groups = [];
    ...
    private checkLevelChange() {
        if (this.gameInstanceService.score > 0 && this.gameInstanceService.score % this.gameInstanceService.levelChangeScore === 0) {
            ...
            const levelText = this.add
                .text(this.game.scale.gameSize.width / 2, this.game.scale.gameSize.height / 2, `Level ${this.gameInstanceService.level}`, {
                    fontSize: '32px',
                    stroke: '#000000',
                    strokeThickness: 5,
                })
